...
<body>
	...
	<nav class="navbar navbar-expand-sm bg-light">
		...
		<div class="collapse navbar-collapse" id="collapsibleNavbar">
			<ul class="navbar-nav">
				<li class="nav-item">
					<a class="nav-link" onclick="firstContent()">Ejercicios</a>
				</li>
				<li class="nav-item">
	...
	<section id="first" class=" containder-fluid row content1">
		<div class="searchBox col-8 col-lg-3"> 
			<h4 class="searchBox-title">Buscar ejercicios</h4>
			<form method="post" action="entrenador.php" class="searchBox-form">
				<select class="form-control" name="tipo_entrenamiento" placeholder="Buscar tipos de dietas">
		...
		<div class=" containder-fluid table-responsive trainingContainer col-lg-9">
			...
			<?php 
				...
				if (isset($_POST['submit'])) {
					...
					$tipo =  mysqli_real_escape_string($conexion,$_POST['tipo_entrenamiento']);

					if ($tipo == 'aerobico' || $tipo == 'anaerobico') {

						$sql = " SELECT * 
									 FROM tabla_ejercicio 
									 WHERE tipo_ejercicio = '$tipo'";

						$result = mysqli_query($conexion,$sql);
						$row = mysqli_fetch_assoc($result);

						echo "<h1>Ejercicios  $tipo"."s ". "</h1>";
						echo "</div>";

						echo "<div class='table-responsive trainingContainer col-lg-12'>";

									// Muestra tabla de ejercicios  
						$resultEjercicios = mysqli_query($conexion,$sql);

						echo "<table class='table trainingTable'>";
						echo "<tr>";
						echo "<td class='tableHeading'>EJERCICIO</td>";
						echo "<td class='tableHeading'>SERIES</td>";
						echo "<td class='tableHeading'>REPETICIONES</td>";
						echo "<td class='tableHeading'>DESCANSO</td>";
						echo "</tr>";
						echo "<tr>";

						while ( $showTables = mysqli_fetch_array($resultEjercicios)) {

							// Selecciona el id de la tabla
							$idTabla = $showTables[0]; 

							// Accede a la tabla ejercicio mediante el id
							$consulta = " SELECT nombre_ejercicio,n_series,n_repeticiones,t_descanso 
										  FROM ejercicio 
	...
	<section class="container-fluid">
		...
		<div id="demo" class="row content2">
			<div class="optionPanel col-lg-3">
				<form method="post" action="entrenador.php" class="form-inline">
	...
	<?php 
		...
		if ($numeroFilas > 0) {
			...
			echo "<section class='set-form-ejercicios'>";
			echo "<form class='set col-md-4' action='../php/asignarEjercicios.php' method='post'>";
			...
			echo "<select name='entrenamiento' class='form-control col-md-12'>";
			echo "<option value='1'>Aeróbico</option>";
			echo "<option value='2'>Anaeróbico</option>";
			echo "</select>";
